...
kadaneImpute <- function(data, kadane.corr, kadane.match = TRUE, donors = 5L, ...){
  ...
  if(kadane.match){
   ...
   for(i in 1:ncol(sets)){
     ...
     #match1 <- mice:::matcher(obs = mahal[set1], mis = mahal[set2], k = donors)
     #match2 <- mice:::matcher(obs = mahal[set2], mis = mahal[set1], k = donors)
     match1 <- mice:::matcher(obs = data[set1, sets[1, i]], mis = imputed[set2, sets[1, i]], k = donors)
     match2 <- mice:::matcher(obs = data[set2, sets[2, i]], mis = imputed[set1, sets[2, i]], k = donors)
